...
for (const filename of ls(
  path.resolve(__dirname, 'geoserver_gmf_geoportal/static-ngeo/js/apps/*.html.ejs')
)) {
  const name = filename.file.substr(0, filename.file.length - '.html.ejs'.length);
  entry[name] = 'geoserver_gmf/apps/Controller' + name + '.js';
  plugins.push(
...
const projectRule = {
  test: /geoserver_gmf_geoportal\/static-ngeo\/js\/.*\.js$/,
  use: {
    loader: 'babel-loader',
    options: {
