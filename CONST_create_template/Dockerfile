...
RUN \
    ...
    c2c-template --vars ${VARS_FILE} \
        --get-config geoserver_gmf_geoportal/config.yaml \
        ${CONFIG_VARS} && \
    pykwalify --data-file geoserver_gmf_geoportal/config.yaml \
        --schema-file CONST_config-schema.yaml && \
    rm CONST_* vars.yaml && \
    qgisserver-plugin-config geoserver_gmf_geoportal/config.yaml ../qgisserver/geomapfish.yaml.tmpl
...
RUN \
    ...
    mv /tmp/config/geoportal/geoserver_gmf_geoportal/ /etc/geomapfish/ && \
